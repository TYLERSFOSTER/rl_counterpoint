"""Resume an interrupted rank-2 tower lineage from checkpoint."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resume an interrupted rank-2 lineage.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    exception_log_path = args.artifact_root / "logs" / f"{args.lineage_id}-rank2-resume.exception.log"
    write_run_heartbeat(
    ...
    try:
        print(f"resuming rank2 lineage: {args.lineage_id}", flush=True)
