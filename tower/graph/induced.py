...
def _enumerate_valid_states(*, spec: TowerGraphSpec) -> tuple[tuple[int, ...], ...]:
    if spec.rank != 2:
        raise ValueError("state enumeration currently requires rank 2")
    states = []
...
def _enumerate_valid_edges(
    *,
    ...
    if spec.rank != 2:
        raise ValueError("edge enumeration currently requires rank 2")
    edges = []
    for source in states:
        for action in action_space(rank=2, max_step_size=spec.max_step_size):
